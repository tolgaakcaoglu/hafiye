...
| P23-FILES-01 | Real local file organize-and-verify fixture | Isolated temporary fixture; direct AIAgent with `terminal` and `file`; create `organized/text` and `organized/media`, move `notes.txt` and `photo.jpg`, then verify with `find` | First run only created directories; second returned without a tool call; both source files remained under `incoming/` | NOT ACCEPTED / KI-041 |
...
| P23-OPENHANDS-01 | Fresh managed OpenHands readiness recheck | `.venv/bin/hafiye runtime openhands doctor` | SDK/source `1.41.0`, `ready=true`, `blockers=[]`; exact P23 fixture delegation remains a separate acceptance step | PASS FOR READINESS |
