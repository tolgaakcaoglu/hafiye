...
| P24-LOCAL-01 | Practical local-first agent route | Capability metadata, local GGUF task evidence, LOCAL_ONLY/OFFLINE fail-closed behavior, and Qwen2 validation-only boundary | Registry state is correct: Qwen2.5-0.5B `validation=true, agent=false`; Qwen3-14B/4B `agent=true, tool_calling=true, validation=false, resource_warning=KI-046`. Source and installed `/usr/bin/hafiye` `LOCAL_ONLY` Qwen3-4B terminal tasks passed through the loopback CUDA endpoint. Browser/coding practical evidence is incomplete; default remains Gemini and Qwen3 remains selectable/non-default | PARTIAL / ROUTE SMOKE PASS; ACCEPTANCE OPEN |
| P24-LOCAL-ROUTE-02 | Selected local route must not inherit global remote endpoint | Policy route resolution, native gateway route construction, and real source/installed CLI `LOCAL_ONLY` task with global Gemini config plus custom Qwen3 route | Red-first policy test reproduced `1 failed, 12 passed`; source fix `10bd0b8f32b99b9a7ad91317dfb2f88b5204b927` resolves the matching loopback custom endpoint. Source CLI returned `P24_LOCAL_ONLY_QWEN3_OK`; installed `/usr/bin/hafiye` returned `P24_INSTALLED_LOCAL_ROUTE_OK`; both exited 0 and restored Gemini/NORMAL | PASS / SOURCE + INSTALLED |
...
root, and matched current install stamp `2c749c93d643`.
...
| P24-PACKAGE-01 | Clean package artifact after browser patch | `cd apps/desktop && npm run pack`; `.venv/bin/python scripts/build_deb.py --output dist/hafiye_0.20.5_amd64.deb --desktop-dir apps/desktop/release/linux-unpacked --json`; `.venv/bin/pytest -q tests/packaging/test_hafiye_deb.py tests/test_packaging_metadata.py`; visible package install; gateway/Desktop restart | Historical package checkpoint `8b7c29aa`; `15 passed`; package contents include updated `tools/hafiye_browser.py` | PASS / HISTORICAL INSTALLED ARTIFACT |
| P24-PACKAGE-02 | Installed package after selected-route endpoint fix | `npm run pack`; `scripts/build_deb.py --json`; root-broker `apt-get --reinstall`; `hafiye package doctor`; installed `/usr/bin/hafiye` route replay | Package manifest source `2c749c93d643`, pinned upstream/baseline values correct; package doctor `OK` with only optional Cargo warning; gateway health HTTP 200; Desktop stamp `2c749c93d643`; installed `LOCAL_ONLY` Qwen3-4B/CUDA task returned `P24_INSTALLED_LOCAL_ROUTE_OK` and route restored | PASS / INSTALLED SOURCE |
...
| P24-UPSTREAM-RECOVERY-02 | Exact historical five-ID comparison after browser hardening | Canonical P23 five-node command | `3 passed, 2 failed`; only accepted whitelist IDs 2 and 3 failed, IDs 1, 4, and 5 passed; this is a reduction within the accepted baseline and no new/different failure was found | ACCEPTED BASELINE REDUCED / NO NEW REGRESSION |
...
| P24-CODE-LOCAL-01 | Installed local coding-agent qualification | Installed `/usr/bin/hafiye ask --provider custom --model .../qwen3-4b-q4_k_m.gguf --toolsets coding` with explicit read-only/no-OpenHands constraints; deterministic project test run separately | Qwen3-4B produced no final agent response before the bounded attempt was stopped; direct Randevu test suite returned `165 passed, 0 failed`; project worktree remained unchanged | NOT ACCEPTED / NO AGENT RESULT |
...
| P24-FINAL-REGRESSION-02 | Post-source targeted regression and accepted upstream comparison | Targeted backend pytest; P24 UI Vitest; `npm run typecheck`; packaging pytest; canonical five-node upstream command; `git diff --check` | Backend `153 passed`; UI `93 passed`; typecheck exit 0; packaging `15 passed`; upstream `3 passed, 2 failed` with only accepted whitelist IDs 2 and 3 failing; diff check clean | PASS / AUTOMATED; P24 ACCEPTANCE STILL OPEN |
...
| P24-QWEN38-PACKAGE-01 | Production package and live installed catalog | `npm run pack`; `scripts/build_deb.py`; root-broker `apt-get --reinstall`; package doctor; authenticated installed `/api/local-runtime/models` | Manifest source `beec5bd9c`; package doctor OK with optional Cargo warning; gateway active/HTTP 200; pinned Qwen3.8 entry reports `downloadable`, `qualification=pending`; installed Desktop stamp is `beec5bd9c061` and size renders as `5.8 GiB` | PASS / INSTALLED CATALOG |
...
| P24-MODEL-I18N-PACKAGE-01 | Installed Turkish Model page | `npm run pack`; `scripts/build_deb.py`; root-broker reinstall; package doctor; isolated real installed Electron render and DOM readback | Package/stamp source `4935c91b7`; doctor OK with optional Cargo warning; target screenshot shows Turkish navigation/runtime/catalog/version copy; `Download verified GGUF`, `built with`, and `NVIDIA NVIDIA` absent; `Doğrulanmış GGUF’u indir` and `llama-server sürüm:` present | PASS / INSTALLED VISUAL |
