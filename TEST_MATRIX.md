...
| P23-QWEN3-01 | Official Qwen3-14B managed model registration and 65K runtime fit | Hafiye-managed download/import of `Qwen/Qwen3-14B-GGUF` Q4_K_M; live llama-server `/v1/models`; `hafiye runtime doctor`; NVIDIA/RAM probes | Revision `530227a7d994db8eca5ab5ced2fb692b614357fd`, 9,001,752,960-byte file, SHA-256 `500a8806e85ee9c83f3ae08420295592451379b4f8cf2d0f41c15dffeb6b81f0`; managed CUDA command reported `n_ctx=65536`, `n_ctx_train=65536`, `ready=true`; full-GPU fit failures are documented diagnostics | PASS / CANDIDATE ONLY |
| P23-QWEN3-02 | Qwen3 embedded Jinja/tool parser | Direct local llama-server chat request with one `open_application` function and `Firefox'u aç.` | HTTP 200; `finish_reason=tool_calls`; function `open_application`; arguments `{"application":"Firefox"}`; 8.41 tok/s at the isolated 4K parser probe | PASS / PARSER SMOKE |
| P23-QWEN3-03 | Real Hafiye agent with managed desktop MCP and Qwen3 | Direct `AIAgent` using model `qwen3-14b-q4_k_m`, managed `hafiye-computer-use-linux` toolset, `/no_think`, `max_tokens=256`, `max_iterations=4`; live Firefox window | `tool_search → tool_describe → list_windows → activate_window`; Firefox became active; `text_response(finish_reason=stop)`, `api_calls=4` | PASS / CANDIDATE ONLY; FULL P23 DEFERRED |
| P23-QWEN3-04 | Qwen3 CLI/oneshot qualification limits | `hafiye ask` at 4K and 64K plus a full managed-MCP oneshot probe | 4K path correctly rejected below Hermes' 64K minimum; generic 64K computer-use path did not select a desktop tool; full managed-MCP oneshot exceeded the 180-second bound under concurrent service load. No full Qwen3 P23 qualification is claimed | DIAGNOSTIC / KI-046 |
