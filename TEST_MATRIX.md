...
| P23-BOOT-POSTREBOOT-01 | P23.1 real reboot/login acceptance | Real `systemctl reboot -i`; new boot ID; `systemctl --user` gateway state/journal; packaged process/window/tray checks; `computer-use-linux windows`; voice/computer doctors | New boot `17a11ea7-41ed-4b74-a4fc-8f4e9c3dc7eb`; gateway `MainPID=8925` active from `06:14:19`, endpoint HTTP 200, voice/computer doctors green, but no packaged Desktop process/app scope/window/tray/Composer appeared after login. Exact packaged launch worked only when started manually for diagnosis | NOT ACCEPTED / KI-013 AUTOSTART BLOCKER |
