...
profile is active, get_hermes_home() should:
  1. STILL return ~/.local/share/hafiye (raising would brick 30+ module-level callers)
...
class TestGetHermesHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        """Classic mode: no active_profile file → silent, returns the Hafiye data root."""
        result = fresh_constants.get_hermes_home()
        assert result == tmp_path / ".local" / "share" / "hafiye"
    ...
    def test_named_profile_unset_home_warns_once(
        ...
        hermes_dir = tmp_path / ".local" / "share" / "hafiye"
    ...
    def test_hermes_home_set_suppresses_warning(
        ...
        profile_dir = tmp_path / ".local" / "share" / "hafiye" / "profiles" / "coder"
        profile_dir.mkdir(parents=True)
        (tmp_path / ".local" / "share" / "hafiye" / "active_profile").write_text("coder\n")
