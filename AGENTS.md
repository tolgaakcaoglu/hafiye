...
- Compute backend policy: `AUTO` is the default.
- `AUTO` priority: NVIDIA present and CUDA available → CUDA; otherwise Vulkan available → Vulkan; otherwise CPU.
- Managed `llama.cpp` backend: primary CUDA, fallback Vulkan, then CPU.
- Managed `whisper.cpp` backend: primary CUDA, fallback Vulkan, then CPU.
- Future Desktop compute selector: Auto / CUDA / Vulkan / CPU.
- On the current host, the expected primary backend is CUDA.
...
- Local Turkish STT: `whisper.cpp` with CUDA primary and Vulkan/CPU fallback.
...
The exact five failures recorded as `ACCEPTED_UPSTREAM_BASELINE` are the P0 regression baseline. The same five after Hafiye changes are not new regressions; fewer failures update the baseline; any new or different failure must be investigated.
