...
from hermes_constants import get_hafiye_state_home, display_hafiye_state_home
...
def tail_log(
    ...
    *,
    ...
    log_path = get_hafiye_state_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Hafiye runs — try 'hafiye chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_hafiye_state_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_hafiye_state_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_hafiye_state_home()}/logs/")
        return

    print(f"Log files in {display_hafiye_state_home()}/logs/:\n")
    found = False
