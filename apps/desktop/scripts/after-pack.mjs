 ...
 *
 * Stamps the Hafiye icon + identity onto the packed Windows executable via
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'hafiye-desktop')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'hafiye-desktop'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); the Hafiye executable keeps the stock Electron icon`)
  }
}
